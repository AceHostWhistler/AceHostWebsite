...
import {
  BlogGuestyPreRelatedBanner,
} from "@/components/blog/BlogGuestyBookingCtas";
...
interface BlogRelatedArticlesProps {
  ...
  articles?: Article[];
  /** Set false on posts that already include multiple Guesty CTAs in the body */
...
export default function BlogRelatedArticles({
  ...
  if (relatedArticles.length === 0) {
    return showBookingCtas ? <BlogGuestyPreRelatedBanner /> : null;
  }

  return (
    <>
      {showBookingCtas ? <BlogGuestyPreRelatedBanner /> : null}

      <section
        aria-labelledby="related-articles-heading"
        className="relative w-screen left-1/2 -translate-x-1/2 bg-neutral-50 border-t border-neutral-200/80 mt-0"
      >
